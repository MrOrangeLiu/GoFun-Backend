    ...
    public class ChatRoomProfile : Profile
    {
        public ChatRoomProfile()
        {
            ...
            CreateMap<ChatRoom, ChatRoomOutputDto>().ForMember(
                    ...
                    dest => dest.Pendings,
                    opt => opt.MapFrom(src => src.UserChatRooms.Where(ucr => ucr.Role == UserChatRoom.UserChatRoomRole.Pending).Select(ucr => ucr.User).ToList())
                ).ForMember(
                    dest => dest.Admins,
                    ...
                    dest => dest.OwnerId,
                    opt => opt.MapFrom(src => src.UserChatRooms.SingleOrDefault(ucr => ucr.Role == UserChatRoom.UserChatRoomRole.Ownder).UserId)
                );
